...
denom_model = None
DENOM_MODEL_PATH = 'denom_model.h5'
...
def init_model():
    ...
    try:
        ...
        if os.path.exists(DENOM_MODEL_PATH):
            denom_model = load_model(DENOM_MODEL_PATH)
            print(f"Loaded denom model from {DENOM_MODEL_PATH}")
    except Exception as e:
...
def predict():
    ...
    if file and allowed_file(file.filename):
        try:
            ...
            # Sub-model: If real and denom_model exists, distinguish between 500 and 2000
            denom_text = ""
            if is_real and denom_model is not None:
                try:
                    # denom_model expects RGB 128x128 
                    ...
                    denom_pred = denom_model.predict(img_rgb_array)
                    denom_prob = float(denom_pred[0][0])
                    # '2000' is class 0, '500' is class 1 (alphabetical order folders)
                    if denom_prob > 0.5:
                        denom_text = "500 Rupee "
                    else:
                        denom_text = "2000 Rupee "
                except Exception as e:
                    print(f"Error predicting denomination: {e}")
            
            # Calculate confidence
            if is_real:
                confidence = probability * 100
                result_text = f"Real {denom_text}Currency"
            else:
